...
try {
  if ((await readFile(marker, 'utf8')).trim() === expectedArchiveSha) process.exit(0)
} catch {}

const partsDir = path.join(root, '.v20-parts')
const partNames = (await readdir(partsDir)).filter((name) => name.endsWith('.txt')).sort()
if (!partNames.length) throw new Error('Club Dynasty v20 archive parts are missing.')

const encoded = (await Promise.all(partNames.map((name) => readFile(path.join(partsDir, name), 'utf8')))).join('')
if (sha256(encoded) !== expectedBase64Sha) throw new Error('Club Dynasty v20 archive text checksum failed.')

const compressed = Buffer.from(encoded, 'base64')
if (sha256(compressed) !== expectedArchiveSha) throw new Error('Club Dynasty v20 archive checksum failed.')
...
console.log('Club Dynasty v20 source prepared successfully.')
